...
def _load_judge_prompts() -> dict[str, str]:
    ...
    # Prefer quoted prompts from judges.md when present
    for key, label in (
        ("builder", "Builder"),
        ("market", "Market"),
        ("impact", "Impact"),
    ):
        m = re.search(rf"{label}.*?Prompt:\s*\"([^\"]+)\"", text, re.I | re.S)
        if m:
...
def _heuristic_judge(key: str, pitch: Pitch) -> tuple[float, str]:
    ...
    if key == "builder":
        ...
        score = _clamp(substance + hits * 4)
        return score, "Feasibility heuristic from pitch substance + build keywords."
    if key == "market":
        hits = sum(1 for w in ("market", "customer", "revenue", "wedge", "compete", "price", "buyer") if w in blob)
        score = _clamp(substance + hits * 5)
        return score, "Market heuristic from wedge/customer language."
    # impact
    hits = sum(
        1
    ...
    score = _clamp(substance + hits * 5 + (8 if pitch.uses_starlink else 0))
    return score, "Impact heuristic from underserved / connectivity language."
...
def score_pitch(pitch: Pitch) -> dict:
    ...
    names = {
        "builder": "The Builder",
        "market": "The Market Shark",
        "impact": "The Impact Judge",
    }
...
# Back-compat alias used by older scaffold imports
def score_pitch_dict(**kwargs) -> dict:
    return score_pitch(Pitch(**kwargs))
